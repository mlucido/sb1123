...
- When in doubt, make a reasonable call and note it in the summary.
...
- One-line status updates during multi-step tasks are fine.

## Autonomous Bug Fixing
- Bug report = fix it. No hand-holding required.
- Use logs, errors, and failing tests as your guide.
- Fix failing CI/Playwright tests without being asked how.
...
- After ANY user correction: immediately write a specific, actionable rule to memory.
- Format: "When [situation], always [action]. Never [anti-pattern]."
- Review project-relevant memory at session start before touching code.
- If you catch yourself about to repeat a past mistake, stop and course-correct silently.
