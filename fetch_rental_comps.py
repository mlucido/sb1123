...
Pulls ALL active rental listings via Redfin's gis-csv endpoint.

Clones fetch_listings.py infrastructure with one key change: &isRentals=true.
Output feeds into listings_build.py as spatial rental comp data for BTR pro forma.
...
# ── Config ──
MAX_RETRIES = 2
MAX_SUBDIVIDE_DEPTH = 4  # Rentals less dense than for-sale
...
def fetch_tile(tile, market, retries=0, try_no_status=False):
    """Fetch rental listings for a geographic tile.
    ...
    url = (
        ...
        f"&page_number=1&uipt=1,2,3,4,5"
        f"&v=8&isRentals=true&user_poly={poly}"
    )
    if not try_no_status:
        url += "&status=9"
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if not text or text.startswith("<!") or text.startswith("{"):
                # If status=9 returned nothing, try without status param
                if not try_no_status:
                    return fetch_tile(tile, market, retries=0, try_no_status=True)
                return []
            ...
            if retries < MAX_RETRIES:
                ...
                time.sleep(wait)
                return fetch_tile(tile, market, retries + 1, try_no_status)
            else:
...
def process_tile(tile, market):
    ...
    if hit_cap and depth >= MAX_SUBDIVIDE_DEPTH:
        print(f"\n      Warning: {tile_label(tile)} still at cap after max depth — some rentals missed")
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"  Redfin {market['name']} — Rental Comps Fetcher")
    if test_mode:
    ...
    print(f"     Unique rentals:      {len(all_data_rows):,}")

    if not all_data_rows:
        print("\n  No rentals fetched. Redfin may be blocking requests.")
        sys.exit(1)
